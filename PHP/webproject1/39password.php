<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		//check for a new password and complete it with confirmed password
		if (!empty($_POST['pass1'])){
			...
			$errors[] = 'You forget to enter yout new password!';
		}
		//if there is no errors
		if (empty($errors)){
			//check that the users entered the right email/password combination
			...
			echo "Error! The forllowing error(s) occured: <br>";
...
<form action="39password.php" method="post">
	...
	<p>Confirm Password : <input type="password" name="pass2" size="10" maxlength="50" value="<?php if(isset($_POST['pass2'])){echo $_POST['pass2']; }?>"></p>
